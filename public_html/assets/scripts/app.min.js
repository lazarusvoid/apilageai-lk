...
function initializeScrollToBottomButton() {
    ...
    chatContainer.addEventListener('scroll', () => {
        ...
        if (currentScrollTop < lastScrollTop) {
            isScrollingUp = true;
            // Show button when scrolling up and not at bottom
            if (!isNearBottom()) {
                showButton();
            }
        }
        // Detect if scrolling down near bottom
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeScrollToBottomButton);
} else {
...
function navigateToHome({ push = true } = {}) {
    ...
    // Show trial banner again if trial has ended
    showTrialBannerIfEnded();
    
    // Update file attach button state
    if (typeof updateFileAttachButtonState === 'function') {
...
function openCanvas({ fullscreen = false } = {}) {
    ...
    // Hide fullscreen button when in fullscreen, show it when not
    const fsBtn = document.getElementById('canvas-fullscreen-btn');
    if (fsBtn) {
        fsBtn.style.display = fullscreen ? 'none' : 'block';
    }
...
function closeCanvas() {
    ...
    // Show fullscreen button when closing canvas
    const fsBtn = document.getElementById('canvas-fullscreen-btn');
    if (fsBtn) {
...
function getStrokeStyleForTool(tool) {
    ...
    if (tool === 'highlight') {
        // Use dark green for highlight in dark mode, otherwise use primary red
        const isDarkMode = document.documentElement.getAttribute('data-theme') === 'dark';
        const highlightColor = isDarkMode ? '#2d5016' : (canvasColor || primary);
        return { color: highlightColor, size: Number(canvasToolSizes.highlight || 18) || 18, alpha: 0.22 };
    }
...
function initCanvasUi() {
    ...
    // Note: menuBtn (toggleCanvasBtn) is now handled by the feature system below
    // menuBtn?.addEventListener('click', () => { ... });
    closeBtn?.addEventListener('click', () => closeCanvas());
    fsBtn?.addEventListener('click', () => {
        // Close sidebar when going fullscreen
        const sidebar = document.getElementById('sidebar');
        if (sidebar && !sidebar.classList.contains('hidden')) {
            sidebar.classList.add('hidden');
            sidebar.setAttribute('aria-hidden', 'true');
            try { sidebar.inert = true; } catch (_) {}
            const toggleSidebarBtn = document.getElementById('toggleSidebar');
            if (toggleSidebarBtn) toggleSidebarBtn.style.display = 'block';
        }
        openCanvas({ fullscreen: true });
    });
...
function renderConversationGallery(conversations) {
    ...
    const appendGroup = (title, items) => {
        ...
        for (const conv of items) {
            ...
            // Publish (owner only) - make visible to all users
            if (isOwner) {
                const publishBtn = document.createElement('button');
                publishBtn.className = 'icon-btn publish-btn';
                publishBtn.title = 'Publish chat';
                publishBtn.innerHTML = '<i class="fa fa-globe"></i>';
                publishBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    await publishChat(String(id), titleText);
                });
                thumb.appendChild(publishBtn);
            }
...
socket.on("authenticated", (data) => {
    if (data.success) {
        ...
        // Show/hide trial ended banner - ONLY if balance is 0 AND trial has ended
        const trialBanner = document.getElementById('trial-ended-banner');
        if (trialBanner) {
            // Banner should show ONLY if balance is 0 AND any trial remaining value is 0
            const hasNoBalance = data.user.balance <= 0;
            const trialEnded = trialRemaining.messages <= 0 || trialRemaining.image_uploads <= 0 || trialRemaining.image_generations <= 0;
        ...
        // Update file attach button state based on image uploads trial
        if (typeof updateFileAttachButtonState === 'function') {
            updateFileAttachButtonState();
        }
        ...
        // Re-check banner and button state now that balance is updated
        if (typeof showTrialBannerIfEnded === 'function') {
            showTrialBannerIfEnded();
        }
        if (typeof updateFileAttachButtonState === 'function') {
            updateFileAttachButtonState();
...
socket.on('available_models', (data) => {
    if (data && data.models) {
        ...
        if (trialBanner) {
            // Banner should show ONLY if balance is 0 AND any trial remaining value is 0
            const hasNoBalance = window.userBalance <= 0;
            const trialEnded = window.trialRemaining.messages <= 0 || window.trialRemaining.image_uploads <= 0 || window.trialRemaining.image_generations <= 0;
            if (hasNoBalance && trialEnded) {
                trialBanner.style.display = 'block';
            } else {
                trialBanner.style.display = 'none';
...
// Publish chat function - opens the share modal instead of separate dialog
async function publishChat(conversationId, chatTitle) {
    // Open the share modal - the publish button handler will be triggered from within the modal
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const publishBtn = document.getElementById('publish-chat-btn');
    const unpublishBtn = document.getElementById('unpublish-chat-btn');
    const publishLinkInput = document.getElementById('publish-link-input');
    const copyPublishLinkBtn = document.getElementById('copy-publish-link-btn');
    const publishLinkContainer = document.getElementById('publish-link-container');
    const publishStatusText = document.getElementById('publish-status-text');

    const loadPublishStatus = async () => {
        if (!shareState.conversationId) return;
        try {
            // Check if conversation is published by looking for publish endpoint
            // For now, just update button visibility based on state
        } catch (error) {
            console.error('Load publish status error:', error);
        }
    };

    publishBtn?.addEventListener('click', async () => {
        if (!shareState.conversationId) {
            setStatus('Open a chat first to publish it.', 'error');
            return;
        }
        if (!viewerIsOwner) {
            setStatus('Only the chat owner can publish it.', 'error');
            return;
        }
        ...
        try {
            const resp = await fetch(`${SHARE_API_BASE}/api/conversations/${shareState.conversationId}/publish`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include'
            });
            ...
            if (data.error) {
                setStatus(data.message || 'Failed to publish chat.', 'error');
                publishBtn.disabled = false;
                publishBtn.innerHTML = '<i class="fa fa-globe" style="margin-right: 6px;"></i> Publish Chat';
                return;
            }

            // Success - show publish link and hide publish button
            publishBtn.style.display = 'none';
            unpublishBtn.style.display = 'inline-flex';
            publishStatusText.textContent = '✓ Chat published! Everyone can now read this conversation.';
            
            if (publishLinkInput && data.publish_link) {
                publishLinkInput.value = data.publish_link;
                publishLinkContainer.style.display = 'block';
            }

            setStatus('Chat published successfully!', 'success');
        } catch (error) {
            console.error('Publish error:', error);
            setStatus('Failed to publish chat: ' + error.message, 'error');
            publishBtn.disabled = false;
            publishBtn.innerHTML = '<i class="fa fa-globe" style="margin-right: 6px;"></i> Publish Chat';
        }
    });
    ...
    unpublishBtn?.addEventListener('click', async () => {
        ...
        try {
            const resp = await fetch(`${SHARE_API_BASE}/api/conversations/${shareState.conversationId}/unpublish`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'include'
            });
            ...
            if (data.error) {
                setStatus(data.message || 'Failed to unpublish chat.', 'error');
                unpublishBtn.disabled = false;
                unpublishBtn.innerHTML = '<i class="fa fa-lock" style="margin-right: 6px;"></i> Unpublish';
                return;
            }

            // Success - hide unpublish button and show publish button
            unpublishBtn.style.display = 'none';
            publishBtn.style.display = 'inline-flex';
            publishStatusText.textContent = 'Click below to publish this chat for all users.';
            publishLinkContainer.style.display = 'none';
            if (publishLinkInput) publishLinkInput.value = '';

            setStatus('Chat unpublished. It is now private.', 'success');
        } catch (error) {
            console.error('Unpublish error:', error);
            setStatus('Failed to unpublish chat: ' + error.message, 'error');
            unpublishBtn.disabled = false;
            unpublishBtn.innerHTML = '<i class="fa fa-lock" style="margin-right: 6px;"></i> Unpublish';
        }
    });

    copyPublishLinkBtn?.addEventListener('click', async () => {
        if (!publishLinkInput || !publishLinkInput.value) return;
        try {
            await navigator.clipboard.writeText(publishLinkInput.value);
            setStatus('Public link copied to clipboard.', 'success');
        } catch (error) {
            console.error('Copy failed:', error);
            setStatus('Copy failed. Try selecting and copying manually.', 'error');
        }
...
// ===============================
// Trial Ended Banner Event Listener
// ===============================
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (bannerUpgradeBtn) {
        bannerUpgradeBtn.addEventListener('click', () => {
            // Open the preference box with billing tab
            openPreferenceBoxWithBillingTab();
        });
    }
...
// Function to show trial banner if trial has ended AND balance is 0
function showTrialBannerIfEnded() {
    const trialBanner = document.getElementById('trial-ended-banner');
    if (trialBanner && window.trialRemaining && window.userBalance !== undefined) {
        // Show banner only if balance is 0 AND any trial remaining value is 0
        const hasNoBalance = window.userBalance <= 0;
        const trialEnded = window.trialRemaining.messages <= 0 || window.trialRemaining.image_uploads <= 0 || window.trialRemaining.image_generations <= 0;
        if (hasNoBalance && trialEnded) {
            trialBanner.style.display = 'block';
...
// Function to manage file attach button state based on trial_remaining.image_uploads AND balance
function updateFileAttachButtonState() {
    const fileAttachBtn = document.getElementById('fileAttach');
    const messageInput = document.getElementById('message-input');
    if (fileAttachBtn && window.trialRemaining && window.userBalance !== undefined) {
        // Disable only if balance is 0 AND image uploads trial has ended
        const hasNoBalance = window.userBalance <= 0;
        const imageUploadsEnded = window.trialRemaining.image_uploads <= 0;
        const shouldDisable = hasNoBalance && imageUploadsEnded;
        
        if (shouldDisable) {
            fileAttachBtn.disabled = true;
            fileAttachBtn.setAttribute('data-tooltip', 'Upgrade to Pro to upload images');
            fileAttachBtn.title = 'Upgrade to Pro to upload images';
            
            // Disable drag and drop by removing event listeners
            if (messageInput) {
            ...
            fileAttachBtn.removeAttribute('data-tooltip');
            fileAttachBtn.title = 'Attach image';
