...
class ChatBubbleManager {
    ...
    generateMessageContent(messageData, options = {}) {
        ...
        if (isStreaming && !finalizeStreaming) {
            ...
            // Handle chat summary for bot messages
            if (!isUserMessage) {
                ...
                if (summaryMatch) {
                    ...
                    if (userBalance < 50) {
                        summaryHTML = `
                            <div class="chat-summary-box" style="position: relative;">
                                <div style="filter: blur(4px);">
                                    🧠 <strong>Chat Summary:</strong> ${summary}
                                </div>
                                <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(132, 128, 128, 0.75); color: #ffc550ff; display: flex; align-items: center; justify-content: center; font-weight: bold; pointer-events: none;">
                                    Account Balance එක 50ට වැඩියෙන් තියෙන අයට Chat Summary එක බලන්න පුළුවන්.
                                </div>
                            </div>
                        `;
                    } else {
                        summaryHTML = `<div class="chat-summary-box">🧠 <strong>Chat Summary:</strong> ${summary}</div>`;
                    }
                }
            }
    ...
    processMessageText(text) {
        ...
        // Extract YouTube and image links
        const videoLinks = [...text.matchAll(/https:\/\/www\.youtube\.com\/watch\?v=([a-zA-Z0-9_-]{11})/g)];
        const imageLinks = [...text.matchAll(/https?:\/\/[\w./-]+\.(?:png|jpe?g|gif|webp)/gi)];

        text = text.replace(/https:\/\/www\.youtube\.com\/watch\?v=([a-zA-Z0-9_-]{11})/g, "");
        text = text.replace(/https?:\/\/[\w./-]+\.(?:png|jpe?g|gif|webp)/gi, "");

        if (videoLinks.length || imageLinks.length) {
            let mediaHTML = '<hr /><div class="resources-section"><div style="margin-top: 10px; font-weight: bold;">Recommended Videos, Images</div><div style="display: flex; flex-wrap: wrap; gap: 16px; margin-top: 8px;">';
            ...
            imageLinks.forEach((link) => {
                mediaHTML += `<a href="${link[0]}" target="_blank"><img src="${link[0]}" style="max-height: 160px; border-radius: 4px;" /></a>`;
            });
    ...
    // Process message content for MathJax and Prism
...
function handleStreamStart(data) {
    ...
    messageInput.value = "Apilage ai is thinking...";
    ...
    chatBubbleManager.createOrUpdateBubble({
        message_id: data.message_id,
        type: 2,
        is_new: data.is_new
    }, {
...
navdropIcon.addEventListener("click", () => {
    navdropMenu.style.display =
        navdropMenu.style.display === "block" ? "none" : "block";
});
        ...
        try {
            ...
            if (fileInput.files[0]) {
                ...
                const uploadResponse = await fetch(
                    "https://apilageai.lk:5001/upload",
                    {
                        method: "POST",
                        body: formData,
                        credentials: "include",
            ...
            // Send message via Socket.IO with STREAMING
...
function updateChatContainer(messages) {
    ...
    // Process MathJax and Prism for all messages
    if (window.MathJax?.typesetPromise) {
...
function processMessageText(text) {
    ...
    const imageLinks = [
        ...text.matchAll(/https?:\/\/[\w./-]+\.(?:png|jpe?g|gif|webp)/gi),
    ];
    ...
    if (videoLinks.length || imageLinks.length) {
        let mediaHTML =
            '<hr /><div class="resources-section"><div style="margin-top: 10px; font-weight: bold;">Recommended Videos, Images</div><div style="display: flex; flex-wrap: wrap; gap: 16px; margin-top: 8px;">';
    ...
    if (mindmapBtn) {
        mindmapBtn.addEventListener("click", () => {
            ...
            if (mindmapInput && openBtn && goBtn) {
                mindmapInput.value = item.txt.trim();
                openBtn.click();
                setTimeout(() => {
                    goBtn.click();
                }, 200);
...
// ==================================================
// Gameyard secction MCQ game
// ==================================================

document.addEventListener('DOMContentLoaded', () => {
    if (typeof MathJax === 'undefined') {
        console.warn("MathJax is not loaded. LaTeX formatting won't work.");
    }
            // --- DOM Elements ---
            const gameIcon = document.getElementById('gameyard-icon');
            const lightbox = document.getElementById('gameyard-lightbox');
            const closeLightboxBtn = document.getElementById('gameyard-close-btn');
            
            // Screens
            const screens = {
                menu: document.getElementById('gameyard-menu-screen'),
                loading: document.getElementById('gameyard-loading-screen'),
                quiz: document.getElementById('gameyard-quiz-screen'),
                summary: document.getElementById('gameyard-summary-screen'),
                error: document.getElementById('gameyard-error-screen')
            };

            // Forms & Buttons
            const quizForm = document.getElementById('gameyard-quiz-form');
            const skipBtn = document.getElementById('gameyard-skip-btn');
            const resetBtn = document.getElementById('gameyard-reset-btn');
            const backToMenuBtn = document.getElementById('gameyard-back-btn');
            const backToMenuFromErrorBtn = document.getElementById('gameyard-back-from-error-btn');
            const langEnBtn = document.getElementById('gameyard-lang-en');
            const langSiBtn = document.getElementById('gameyard-lang-si');
            const difficultyEasyBtn = document.getElementById('gameyard-difficulty-easy');
            const difficultyMediumBtn = document.getElementById('gameyard-difficulty-medium');
            const difficultyHardBtn = document.getElementById('gameyard-difficulty-hard');
            
            // Quiz UI Elements
            const scoreEl = document.getElementById('gameyard-score');
            const questionCounterEl = document.getElementById('gameyard-q-counter');
            const questionTextEl = document.getElementById('gameyard-q-text');
            const optionsContainer = document.getElementById('gameyard-options-container');

            // Summary UI Elements
            const finalScoreEl = document.getElementById('gameyard-final-score');
            const summaryDetailsEl = document.getElementById('gameyard-summary-details');

            // Error UI
            const errorMessageEl = document.getElementById('gameyard-error-message');
            ...
            const fetchQuestions = async (subject, grade, language, difficulty) => {
                showScreen('loading');

                try {
                    const response = await fetch('https://apilageai.lk/api/quizdata.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ subject, grade, language, difficulty })
                    });
            ...
            const displayQuestion = () => {
                ...
                if (typeof MathJax !== 'undefined') MathJax.typesetPromise([questionTextEl]);
            ...
            const showSummary = () => {
                ...
                quizData.forEach((q, index) => {
                    ...
                    const wasSkipped = userAnswerIndex === -1;
                    
                    let statusIcon, cardClass;

                    if (wasSkipped) {
                        statusIcon = `<i class="fas fa-forward"></i> Skipped`;
                        cardClass = 'skipped-card';
                    } else if (isCorrect) {
                        statusIcon = `<i class="fas fa-check-circle"></i> Correct`;
                         cardClass = 'correct-card';
                    } else {
                        statusIcon = `<i class="fas fa-times-circle"></i> Incorrect`;
                        cardClass = 'incorrect-card';
                    }
                    
                    let userAnswerText = 'N/A';
                    if (!wasSkipped && q.options[userAnswerIndex]) {
                        userAnswerText = q.options[userAnswerIndex];
                    }

                    const detailDiv = document.createElement('div');
                    detailDiv.className = `summary-card ${cardClass}`;
                    detailDiv.innerHTML = `
                        <p><strong>${index + 1}. ${q.question}</strong></p>
                        <p>Your answer: <span>${userAnswerText}</span></p>
                        <p>Correct answer: <span>${q.options[q.correctAnswerIndex]}</span></p>
                        <p class="explanation"><strong>Explanation:</strong> ${q.explanation}</p>
                        <div class="status-icon">${statusIcon}</div>`;
                    summaryDetailsEl.appendChild(detailDiv);
                    if (typeof MathJax !== 'undefined') MathJax.typesetPromise([detailDiv]);
                });
            };
            ...
            const resetGame = () => {
                ...
                document.getElementById('gameyard-grade').value = "O/L (Ordinary Level)";
                
                showScreen('menu');
            };

            // --- Event Listeners ---
            gameIcon.addEventListener('click', () => toggleLightbox(true));
            closeLightboxBtn.addEventListener('click', () => toggleLightbox(false));
            ...
            quizForm.addEventListener('submit', (e) => {
                e.preventDefault();
                const subject = document.getElementById('gameyard-subject').value.trim();
                const grade = document.getElementById('gameyard-grade').value;

                if (subject && grade) {
                    fetchQuestions(subject, grade, selectedLanguage, selectedDifficulty);
                } else {
            ...
            backToMenuFromErrorBtn.addEventListener('click', resetGame);
...
// ==================================================
// The setting box and ui setting
// ==================================================
...
// Open on user info click
document.getElementById('sidebarUserInfo').addEventListener('click', toggleSettingBox);

// Close button inside the settingbox
document.getElementById('closeSettingBox').addEventListener('click', toggleSettingBox);
...
const rechargeBtn = document.getElementById('rechargeButton');

rechargeSlider.addEventListener('input', () => {
  const amount = rechargeSlider.value;
  rechargeValue.textContent = amount;
  rechargeBtn.href = `https://apilageai.lk/pay/${amount}`;
});

document.getElementById('closeBannerBtn')?.addEventListener('click', () => {
  document.getElementById('lowBalanceBanner').style.display = 'none';
});
